...
public class MetricIngestService {
    ...
    public void ingestMetric(MetricIngestEvent metricsRequest) {
        webClient.post()
                .uri("/api/metrics")
                .headers(headers -> {
                ...
                .doOnSuccess(response -> log.info("metric ingest successful ..."))
                .doOnError(error -> log.error("Error while ingesting metrics: {}", error.getMessage()))
                .subscribe();
    }
}
